...
/*
	Código adaptado e EXPANDIDO de:
...
//----------------------------------------------------------------------------------------------
// Inicialização dos pinos de IO do ATSAM3X
//----------------------------------------------------------------------------------------------
void inic_IO_LCD()
{
	// CLK (13 - PB27)
	...
	// DIN (12 - PD8), DC (11 - PD7)
	PIOD->PIO_PER = PIO_PD8 | PIO_PD7;
	PIOD->PIO_OER = PIO_PD8 | PIO_PD7;
	PIOD->PIO_PUDR = PIO_PD8 | PIO_PD7;
		
	// CE (10 - PC29), RST (9 - PC21)
	PIOC->PIO_PER = PIO_PC29 | PIO_PC21;
	PIOC->PIO_OER = PIO_PC29 | PIO_PC21;
	PIOC->PIO_PUDR = PIO_PC29 | PIO_PC21;
...
void goto_XY(uint32_t x, uint32_t y)  // 0<=x<=83  0<=y<=5
{
	...
	cmd_LCD(0x80 | x);		// Column. 0 - 83
	cmd_LCD(0x40 | y);		// 6 linhas (Bancos) com 8 bits de altura cada

	indice_fb =  x + (84*y);		// indice para ser empregado no fb
}
//----------------------------------------------------------------------------------------------- 
void bitmap_LCD(const unsigned char my_array[])		// desenha em todo o LCD
{
	uint32_t i;
...
{
	fb[indice_fb] = 0x00;
	data_LCD(fb[indice_fb]);	//Blank vertical line padding
	indice_fb++;

	for(uint32_t i = 0 ; i < 5 ; i++)
	{
		fb[indice_fb] = ASCII[character - 0x20][i];
		data_LCD(fb[indice_fb]);			//0x20 is the ASCII character for Space (' '). The font table starts with this character
		indice_fb++;
	}
...
void string_LCD_Nr(char * msg, uint32_t valor,			// escreve uma mensagem com um número
								uint32_t quant2Print)	// quant2Print é o número de digitos para serem impressos
														// se quant2Print = 0, imprime todos os digitos existentes no número.
{
	uint32_t n=0, i;
...
void limpa_LCD() 
{
	...
	goto_XY(0,0);			//After we clear the display, return to the home position
}
//----------------------------------------------------------------------------------------------- 
// Desenha pixel 
//----------------------------------------------------------------------------------------------- 
				   ...
				   uint32_t propr)			/* 0 =  apaga pixel, 1 = liga pixel				*/
{
	...
	goto_XY(x,y/8);				/* y/8 seleciona um dos 6 bancos verticais	*/
	data_LCD(fb[i]);					   
}
//-----------------------------------------------------------------------------------------------
...
*		------------p2
*/
void desenha_retangulo(struct  pontos_t *p,		/*  p.x1=x1, p.y1=y1, p.x2=x2, p.y2=y2, passagem dos pontos por struct	*/
...
//--------------------------------------------------------------------------------------------------------------
/* FUNÇÃO PARA DESENHO DE UMA FIGURA em qualquer ponto da tela e qualquer parte da figura
*
...
*	  -------------
*
*	Redefinindo o tamanho da figura (menor) - [ p.x2 != 0 e p.y2 !=0 com p.x3 = 0 e p.y3 = 0]
*
*		p.x2
*	 P1 --------
*    |xxxxxxxxxx| p.y2
*	  ----------
*
*	Imprimindo a partir de um ponto no meio da figura - [ p.x2 == 0 e p.y2 ==0 com p.x3 != 0 e p.y3 != 0]
*
*	      ___p.x3___
*	       ----------------
* p.y3|   |        P1______|
				...
				const struct figura_t *f)	// ponteiro para a figura definido pelo tipo figura_t
{
	...
	if((p->x2!=0) && (p->y2!=0)) 
	{
		if(p->x2 < larg) // correcao se valor maior que o permitido
			larg = p->x2;
		if(p->y2 < alt)
			alt = p->y2;
	}
	if((p->x3!=0) && (p->y3!=0)) 
	{
		if(p->x3 >= larg) // volta ao valor default para impressao
			p->x3 = 0;
		if(p->y3 >= alt)
			p->y3 = 0;
	}
	...
	//------------------------------------------------------------------------------------------
	if((p->x3==0) || (p->y3==0))	// impressao normal a partir do canto superior esquerdo da figura
	{
		for(j=0; j<alt; j++)
		{
			for(i=0; i<larg; i++)
	...
	else	// impressao a partir de um ponto no meio da figura
	{
		for(j=p->y3; j<alt; j++)
		{
			for(i=p->x3; i<larg; i++)
			{
				desenha_pixel(x+i-p->x3, y+j-p->y3, tst_bit(f->pixels[i + ((j/8)*f->largura)], j%8));
			}
		}
	}
	//------------------------------------------------------------------------------------------	
...
// ----------------------------------------------------------------------------------------------
// Desenha o frame buffer todo no LCD
//-----------------------------------------------------------------------------------------------
//void fb2LCD()		// desenha em todo o LCD
//{
//	uint32_t i;
