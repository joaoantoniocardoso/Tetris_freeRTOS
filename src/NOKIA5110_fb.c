...
{	
	...
	//xFB_Mutex = xSemaphoreCreateMutex();
	vSemaphoreCreateBinary(xFB_Mutex);
}
//----------------------------------------------------------------------------------------------- 
...
void escreve2fb(unsigned char imagem[])				// altera o frame buffer, uso para cópia de figura
{
	...
	if( xSemaphoreTake(xFB_Mutex, (portTickType) 100) == pdTRUE){
		for(i=0; i<504; i++)	fb[i]= imagem[i];
		xSemaphoreGive(xFB_Mutex);
	}
